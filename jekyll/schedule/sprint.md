...
| [Sprint 6 — 제출·대회]({{ "/docs/sprints/s6.html" | relative_url }}) | 9/1 ~ 9/2 | 최종 결과물 제출, 발표·실제 플레이 시연 | ✅ 완료 — 🏆 **우수상(고용노동부장관상)** |
...
<div class="kanban-card"><p class="card-title">라이선스 작업 커밋·푸시 — 화면 배포·버전 로그는 끝, 형상 관리 반영만 잔여</p><div class="card-meta"><span class="tag tag-shin">신채연</span><span class="tag tag-p1">P1</span></div></div>
<div class="kanban-card"><p class="card-title">실내 NPC 말 걸기 자동 점검 실패(3회) — 자동화 입력 한계인지 실제 결함인지 판별</p><div class="card-meta"><span class="tag tag-shin">신채연</span><span class="tag tag-p1">P1</span></div></div>
<div class="kanban-card"><p class="card-title">favicon 부재 404 — 기존 로고 에셋으로 붙일지 결정</p><div class="card-meta"><span class="tag tag-shin">신채연</span><span class="tag tag-p1">P1</span></div></div>
</div>
...
<div class="kanban-card"><p class="card-title">OCR 입력 상한 — 9/2 릴리스 반영(BE v0.16.30), 연속 3건 실측 peak 1.34GB·문서 점검 테스트 71개 통과</p><div class="card-meta"><span class="tag tag-lee">이은상</span><span class="tag tag-p0">P0</span></div></div>
<div class="kanban-card"><p class="card-title">라이선스 정리 — 재배포 금지 에셋 히스토리 제거(82커밋 재작성·잔존 0) + 고지 페이지 <code>/licenses</code>(FE v0.34.0)</p><div class="card-meta"><span class="tag tag-shin">신채연</span><span class="tag tag-p0">P0</span></div></div>
<div class="kanban-card"><p class="card-title">🏆 해커톤 본선 — <strong>우수상(고용노동부장관상) 수상</strong> (9/4)</p><div class="card-meta"><span class="tag tag-all">전원</span><span class="tag tag-p0">P0</span></div></div>
</div>

</div>
</div>

> 8/30 기준 대비 이동: 8/30에 To Do로 올렸던 **시연 배포 실행·검증(B안)** 과 **프론트 프로덕션 빌드**는 8/31에 실도메인 배포까지 마쳐 Done으로 올라갔다(노출 대응 — 판정 API 인증·API 자동 문서 차단·CORS·쿠키 Secure·OAuth 리디렉트 포함). 9/1에는 **방어전 턴 지연(임베딩 429 고정 25초 대기 → 지수 백오프, 28초 → 중앙값 0.4초)** 과 **리포트 PDF 인쇄 복구**가 Done으로 들어왔고, **OCR 입력 상한**은 코드·테스트가 통과했으나 커밋·버전 반영 전이라 In Progress에 둔다. 배포가 끝나면서 To Do에는 **외부망 실동작 확인 + 긴 요청 터널 통과**(본선 1일차)와 **실제 폰 사진 문서 점검 왕복 재확인**이 새로 올라왔다. **최종 결과물 패키징**은 별도 빌드 산출물을 넘기는 대신 실도메인 배포로 갈음하기로 정리해 Done으로 옮겼다 — 심사위원이 브라우저에서 바로 플레이한다. **발표자료·시연 시나리오**도 최종본이 나와 Done으로 이동했고, 네트워크 사고에 대비한 **시연 영상**(유튜브 전체 완주 롱폼 1편 + 기능별 숏폼 4편)이 함께 확보됐다. 연출 폴리싱(응답 스트리밍·클리어 사운드)과 선택지 시뮬레이션은 그대로 잔여다. 9/2에는 **OCR 입력 상한**이 릴리스 반영(BE v0.16.30)되어 Done으로 옮겨졌고, **라이선스 정리**(공개 저장소 재배포 금지 에셋 히스토리 제거 + 오픈소스 고지 페이지 `/licenses`, FE v0.34.0)가 Done으로 새로 들어왔다. 그리고 **9/4, 해커톤 본선이 끝났다 — 우수상(고용노동부장관상) 수상**으로 보드의 마지막 P0 카드가 Done에 올라갔다.
